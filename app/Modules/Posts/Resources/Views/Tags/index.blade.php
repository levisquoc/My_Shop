                ...
                <div class="card-body">
                    ...
                    <div class="icons">
                        ...
                        @if(Creeper::canOrabort('create-tags'))
                            <a href="{{route('blog.tags.create')}}" class="btn btn-outline-success btn-rounded"><i
                                        class="fa fa-plus"></i> Add new</a>
                        @endif
                        @if(Creeper::canOrabort('delete-tags') && count($trashed) != 0)
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <form id="delete_form" action="{{route('blog.tags.index')}}" method="post">
                            {{csrf_field()}}
                            ...
                            <table id="tableTrash" class="table table-bordered table-striped mt-2">
                                ...
                                @foreach($trashed as $data)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{route('blog.tags.remove',$data->id)}}"
                                               class="btn btn-circle btn-danger remove" data-id="{{ $data->id }}"
                                               data-toggle="tooltip" data-original-title="Delete"> <i
                                                        class="fa fa-close"></i> </a>

                                            <a href="{{route('blog.tags.restore',$data->id)}}"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">Confirm remove tag</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    </div>
                    <div class="modal-body">
                        <p><b>Are you sure to remove <strong id="target-name"></strong> tag ?</b></p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default waves-effect" data-dismiss="modal">Close</button>
                        <form id="remove_form" action="{{route('blog.tags.index')}}" method="post">
                            {{csrf_field()}}
...
@push('scripts')
    ...
    <script type="text/javascript">
        var table = $('.table-index').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: "{{route('blog.tags.serverside.gettags')}}",
            "columns": [
        ...
        function deleteSelected() {
            ...
            if (data.length === 0) {
                swal({
                    title: "Oops...",
                    text: "Please select tags for use this action",
                    type: "error"
                });
            }
            else {
        ...
        function deleteTrashSelected() {
            ...
            if (data.length === 0) {
                ...
                $('.preloader').show();
                axios.get('{{route('blog.tags.deleteTrashSelected')}}', {
                    params: {
                        data: data
                    }
                })
        ...
        function restoreTrashSelected() {
            ...
            if (data.length === 0) {
                ...
                $('.preloader').show();
                axios.get('{{route('blog.tags.restoreTrashSelected')}}', {
                    params: {
                        data: data
                    }
                })

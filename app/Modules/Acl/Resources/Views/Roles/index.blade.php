                        ...
                        <div class="btn-group">
                            ...
                            <div class="dropdown-menu">
                                <a class="dropdown-item" href="javascript:void (0);" onclick="location.reload();">Reload
                                    this page</a>
                                @if(Creeper::canOrabort('delete-roles'))
                                    <a class="dropdown-item" href="javascript:void (0);" onclick="deleteSelected();">Delete
                                        selected items</a>
                                @endif
                            </div>
                        </div>
                        @if(Creeper::canOrabort('create-roles'))
                            <a href="{{route('acl.roles.create')}}" class="btn btn-outline-success btn-rounded"><i
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <form id="delete_form" action="{{route('acl.roles.index')}}" method="post">
                        {{csrf_field()}}
...
@push('style')
    ...
    <style>
        ...
        @endif
        @if(!Creeper::canOrabort('update-roles'))
            .edit {
            display: none;
        }

        @endif
        @if(!Creeper::canOrabort('read-roles'))
            .read {
            display: none;
        }
        @endif
...
@push('scripts')
    ...
    <script type="text/javascript">
        var table = $('.table').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: "{{route('acl.roles.serverside.getroles')}}",
            columns: [
        ...
        function deleteSelected() {
            ...
            if (data.length === 0) {
                swal({
                    title: "Oops...",
                    text: "Please select roles for use this action",
                    type: "error"
                });
            }
            else {
                $('.preloader').show();
                axios.get('{{route('acl.roles.deleteSelected')}}', {
                    params: {
                        data: data
                    }
                })

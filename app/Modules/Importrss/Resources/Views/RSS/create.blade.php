...
@section('content')
    <div class="row">
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">Create New Link <a class="btn btn-danger btn-circle pull-right"
                                                              data-toggle="tooltip" title="Back"
                                                              href="{{route('rss.rss.index')}}"><i
                                    class="fa fa-times"></i></a></h4>

                    <form class="form-horizontal form-material row" action="{{route('rss.rss.store')}}" method="post">
                    {{csrf_field()}}
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-12">
                                                    <input type="text" name="rss_link"
                                                           ...
                                                           value="{{old('link_rss')}}" id="name">
                                                </div>
                                            </div>

                                            @php $pages_target = config('creeper.page_target'); @endphp
                                            <div class="form-group">
                                                    ...
                                                    <select class="form-control" id="page_target" name="page_target">
                                                        ...
                                                        @if(count($pages_target) > 0)
                                                            @foreach($pages_target as $key => $page_target)
                                                                ...
                                                                <option value="{{$page_target}}" {{ (old('page_target') == $key ? 'selected':'') }}>{{$page_target}}</option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                                </div>

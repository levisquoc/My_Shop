                        ...
                        <div class="btn-group">
                            ...
                            <div class="dropdown-menu">
                                <a class="dropdown-item" href="javascript:void (0);" onclick="location.reload();">Reload
                                    this page</a>
                                @if(Creeper::canOrabort('delete-pages'))
                                    <a class="dropdown-item" href="javascript:void (0);" onclick="deleteSelected();">Delete
                                        selected items</a>
                                @endif
                            </div>
                        </div>
                        @if(Creeper::canOrabort('create-pages'))
                            <a href="{{route('pages.create')}}" class="btn btn-outline-success btn-rounded"><i
...
@section('modals')
    @if(Creeper::canOrabort('delete-pages'))
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <form id="delete_form" action="{{route('pages.index')}}" method="post">
                            {{csrf_field()}}
        ...
        <!-- !Delete form -->
    @endif
    @if(Creeper::canOrabort('read-pages'))
        <!-- View profile -->
...
@push('style')
    ...
    <style>
        ...
        @endif
        @if(!Creeper::canOrabort('update-pages'))
            .edit {
            display: none;
        }
...
@push('scripts')
    ...
    <script type="text/javascript">
        var table = $('.table').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: "{{route('pages.serverside.getData')}}",
            "columns": [
        ...
        function deleteSelected() {
            ...
            if (data.length === 0) {
                ...
                $('.preloader').show();
                axios.get('{{route('pages.deleteSelected')}}', {
                    params: {
                        data: data
                    }
                })

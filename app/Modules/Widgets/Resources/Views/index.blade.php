                        ...
                        <div class="btn-group">
                            ...
                            <div class="dropdown-menu">
                                <a class="dropdown-item" href="javascript:void (0);" onclick="location.reload();">Reload
                                    this widget</a>
                                @if(Creeper::canOrabort('delete-widgets'))
                                    <a class="dropdown-item" href="javascript:void (0);" onclick="deleteSelected();">Delete
                                        selected items</a>
                                @endif
                            </div>
                        </div>
                        @if(Creeper::canOrabort('create-widgets'))
                            <a href="{{route('widgets.create')}}" class="btn btn-outline-success btn-rounded"><i
...
@section('modals')
    @if(Creeper::canOrabort('delete-widgets'))
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <form id="delete_form" action="{{route('widgets.index')}}" method="post">
                            {{csrf_field()}}
        ...
        <!-- !Delete form -->
    @endif
    @if(Creeper::canOrabort('read-widgets'))
        <!-- View profile -->
...
@push('style')
    ...
    <style>
        ...
        @endif
        @if(!Creeper::canOrabort('update-widgets'))
            .edit {
            display: none;
        }
...
@push('scripts')
    ...
    <script type="text/javascript">
        var table = $('.table').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: "{{route('widgets.serverside.getData')}}",
            "columns": [
        ...
        function deleteSelected() {
            ...
            if (data.length === 0) {
                ...
                $('.preloader').show();
                axios.get('{{route('widgets.deleteSelected')}}', {
                    params: {
                        data: data
                    }
                })
